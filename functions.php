...
function monomyth_ae_module_run( $args, $module ) {
    ...
    if ( function_exists( '\aw2_library::module_run' ) ) {
        return \aw2_library::module_run( $args, $module );
    }
    
    // Fallback: try shortcode
    $post_type = isset( $args['post_type'] ) ? $args['post_type'] : '';
    $shortcode = '[' . $module;
    if ( ! empty( $post_type ) ) {
        $shortcode .= ' post_type="' . esc_attr( $post_type ) . '"';
    }
    $shortcode .= ' /]';
...
function monomyth_get_app_collection_post_type() {
    ...
    if ( function_exists( 'aw2_library::get_array_ref' ) ) {
...
function monomyth_render_awesome_block( $attributes, $content ) {
    ...
    if ( ! empty( $wrapper_class ) ) $classes[] = $wrapper_class;
    if ( ! empty( $class_name ) ) $classes[] = $class_name;
